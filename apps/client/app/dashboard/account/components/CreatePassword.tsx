  ...
  const createPwd = async (e: any) => {
    ...
    try {
      ...
      const req: AxiosResponse = await axios.post(
        process.env.NEXT_PUBLIC_DASHBOARD_CREATE_PWD_URL as string,
        formData,
        { headers: { Authorization: `Bearer ${Cookie.get('t')}` } }
      );
      setNotification('success', 'Password was created');
  ...
  return (
    <>
      ...
      <p className="text-slate-500 my-2">
        Account auth by google or github are passwordless, thus the user can
        only log in using google or github. To log in with SSO youll need to
        create password below
      </p>
      <form className="w-full my-6" onSubmit={(e) => createPwd(e)}>
